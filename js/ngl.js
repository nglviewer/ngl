...
NGL.TubeImpostorBufferBAK = function ( position, color, radius, segments ) {
    ...
    for( var v = 0; v < n; v++ ) {
        ...
        for ( j = 1; j <= segments; j++ ) {
            ...
            cylRadius[ l + j1 ] = rad;//rad - (j*(radPrev-rad))/segments;
    ...
    new NGL.CylinderImpostorBuffer( cylFrom, cylTo, cylColor, cylColor, cylRadius, nx );
    // new NGL.CylinderGroup( cylFrom, cylTo, cylColor, cylRadius );
    // new NGL.SphereImpostorBuffer( cylFrom, cylColor, cylRadius, false );
    // new NGL.SphereImpostorBuffer( cylTo, cylColor, cylRadius, false );
    // new NGL.SphereImpostorBuffer( spherePos, cylColor, cylRadius, false );
...
NGL.SheetBuffer = function( position, normal, dir, color, size ){
    ...
    geometry.addAttribute( 'position', Float32Array, n2, 3 );
    geometry.addAttribute( 'inputDir', Float32Array, n2, 3 );
    geometry.addAttribute( 'inputSize', Float32Array, n2, 1 );
    geometry.addAttribute( 'normal', Float32Array, n2, 3 );
    //geometry.addAttribute( 'inputNormal', Float32Array, n2, 3 );
    geometry.addAttribute( 'inputColor', Float32Array, n2, 3 );
    ...
    geometry.addAttribute( 'index', Uint16Array, (n2-2)*3, 1 );
    var indices = geometry.attributes.index.array;

    geometry.offsets = NGL.calculateOffsets( (n-1)*4, 2, 4 );
    ...
    for( var v = 0; v < n; ++v ){
        ...
        for( i = 0; i<2; ++i ){
            p = k + 3 * i;
            aPosition[ p + 0 ] = position[ k + 0 ];
            aPosition[ p + 1 ] = position[ k + 1 ];
            aPosition[ p + 2 ] = position[ k + 2 ];

            inputNormal[ p + 0 ] = normal[ k + 0 ];
            inputNormal[ p + 1 ] = normal[ k + 1 ];
            inputNormal[ p + 2 ] = normal[ k + 2 ];

            inputColor[ p + 0 ] = color[ k + 0 ];
            inputColor[ p + 1 ] = color[ k + 1 ];
            inputColor[ p + 2 ] = color[ k + 2 ];

            inputSize[ l + i ] = size[ v ];
        }

        inputDir[ k + 0 ] = dir[ k + 0 ];
        inputDir[ k + 1 ] = dir[ k + 1 ];
        inputDir[ k + 2 ] = dir[ k + 2 ];

        inputDir[ k + 3 ] = -dir[ k + 0 ];
        inputDir[ k + 4 ] = -dir[ k + 1 ];
        inputDir[ k + 5 ] = -dir[ k + 2 ];
    }
    ...
    for( var v = 0; v < n; ++v ) {
        ...
        indices[ ix + 1 ] = (it + 3) % chunkSize;
        indices[ ix + 2 ] = (it + 1) % chunkSize;

        indices[ ix + 3 ] = (it + 0) % chunkSize;
        indices[ ix + 4 ] = (it + 2) % chunkSize;
        indices[ ix + 5 ] = (it + 3) % chunkSize;
    }
...
NGL.SheetBufferBAK = function( position, color, size, segments ){

    // calc spline path
    // render indexed quads (4 points, 2 triangles)
    // push 2 vertices to shader, extrude in shader based on mapping
    ...
    var numpoints = segments*n+1;
    var frames = new THREE.TubeGeometry.FrenetFrames( path, numpoints, false ),
        tangents = frames.tangents,
        normals = frames.normals,
        binormals = frames.binormals;

    var points2 = new Float32Array( numpoints*3 );
    var normals2 = new Float32Array( numpoints*3 );
    var binormals2 = new Float32Array( numpoints*3 );
    var i3, p;
    for ( var i = 0; i < numpoints; i++ ) {
        i3 = i*3;
        p = path.getPointAt( i / ( numpoints - 1 ) );
        points2[ i3 + 0 ] = p.x;
        points2[ i3 + 1 ] = p.y;
        points2[ i3 + 2 ] = p.z;
        normals2[ i3 + 0 ] = normals[ i ].x;
        normals2[ i3 + 1 ] = normals[ i ].y;
        normals2[ i3 + 2 ] = normals[ i ].z;
        binormals2[ i3 + 0 ] = binormals[ i ].x;
        binormals2[ i3 + 1 ] = binormals[ i ].y;
        binormals2[ i3 + 2 ] = binormals[ i ].z;
    }

    console.log( "frenet", n, nx, nx2, segments, frames );


    // make geometry and populate buffer
    geometry = new THREE.BufferGeometry();

    geometry.addAttribute( 'position', Float32Array, nx2, 3 );
    geometry.addAttribute( 'inputDir', Float32Array, nx2, 3 );
    geometry.addAttribute( 'inputSize', Float32Array, nx2, 1 );
    geometry.addAttribute( 'normal', Float32Array, nx2, 3 );
    //geometry.addAttribute( 'inputNormal', Float32Array, nx2, 3 );
    geometry.addAttribute( 'inputColor', Float32Array, nx2, 3 );
    ...
    geometry.addAttribute( 'index', Uint16Array, (nx2-2)*3, 1 );
    var indices = geometry.attributes.index.array;

    geometry.offsets = NGL.calculateOffsets( (nx-1)*4, 2, 4 );
    console.log(geometry.offsets);

    var x, y, z;
    var r, g, b;
    var i, j, k, l, m, p, o, s, u;
    var point, dir, normal, t;
    var chunkSize = NGL.calculateChunkSize( 4 );
    ...
    for( var v = 0; v < n; ++v ){
        ...
        for( j = 0; j < segments; ++j ){
            ...
            for( i = 0; i<2; ++i ){
                p = s + 3 * i;
                aPosition[ p + 0 ] = point.x;
                aPosition[ p + 1 ] = point.y;
                aPosition[ p + 2 ] = point.z;

                inputNormal[ p + 0 ] = normal.x;
                inputNormal[ p + 1 ] = normal.y;
                inputNormal[ p + 2 ] = normal.z;

                inputColor[ p + 0 ] = r;
                inputColor[ p + 1 ] = g;
                inputColor[ p + 2 ] = b;

                // inputSize[ o + i ] = size[ v ];
                inputSize[ o + i ] = size[ v ] - (j*(radPrev-size[ v ]))/segments;
            }
    ...
    new NGL.BufferVectorHelper( points2, normals2, new THREE.Color("rgb(255,0,0)") );
    new NGL.BufferVectorHelper( points2, binormals2, new THREE.Color("rgb(255,255,0)") );
...
NGL.QuadricImpostorBuffer = function( position, color ){
...
NGL.ParticleBuffer = function ( position, color, size ) {
    ...
    particle = new THREE.ParticleSystem( geometry, material, THREE.LinePieces );
