...
UI.Integer=function(a){UI.Element.call(this);var b=this,c=document.createElement("input");c.className="Number";c.value="0.00";c.addEventListener("keydown",function(a){a.stopPropagation()},!1);this.min=-Infinity;this.max=Infinity;this.step=1;this.dom=c;this.setValue(a);var d=document.createEvent("HTMLEvents");d.initEvent("change",!0,!0);var e=0,f=0,h=[0,0],g=[0,0],l=function(a){var k=c.value;h=[a.clientX,a.clientY];e+=h[0]-g[0]-(h[1]-g[1]);var m=50;a.shiftKey&&(m=5);a.ctrlKey&&(m=500);c.value=Math.min(b.max,
Math.max(b.min,f+e/m*b.step))|0;k!==c.value&&c.dispatchEvent(d);g=[a.clientX,a.clientY]},k=function(a){document.removeEventListener("mousemove",l,!1);document.removeEventListener("mouseup",k,!1);2>Math.abs(e)&&(c.focus(),c.select())};c.addEventListener("mousedown",function(a){a.preventDefault();e=0;f=parseFloat(c.value);g=[a.clientX,a.clientY];document.addEventListener("mousemove",l,!1);document.addEventListener("mouseup",k,!1)},!1);c.addEventListener("change",function(a){a=parseInt(c.value);!1===
...
UI.PopupMenu=function(a,b,c){c=c||"scrollParent";UI.Panel.call(this);var d=new UI.Icon(a||"bars"),e=(new UI.OverlayPanel).setDisplay("none").attach(this.dom);a=(new UI.Panel).setBorderBottom("1px solid #555").setMarginBottom("10px").setHeight("25px");a.add((new UI.Icon("times")).setFloat("right").setCursor("pointer").onClick(function(){e.setDisplay("none")})).add(new UI.Text(b));e.add(a);var f;d.setTitle("menu");d.setCursor("pointer");d.onClick(function(a){"block"===e.getDisplay()?(e.setDisplay("none"),
f.destroy()):(e.setDisplay("block"),f=new Tether({element:e.dom,target:d.dom,attachment:"top right",targetAttachment:"top left",offset:"0px 5px",constraints:[{to:c,attachment:"element",pin:["top","bottom"]}]}),f.position())});this.add(d);this.setClass("").setDisplay("inline");this.icon=d;this.panel=e;this.entryLabelWidth="100px";return this};UI.PopupMenu.prototype=Object.create(UI.Panel.prototype);
...
NGL.MenubarFileWidget=function(a){var b="pdb ent gro cif mmcif mrc ccp4 map obj ply ngl ngz gz lzma bz2 zip".split(" "),c=b+["ngl"],d=document.createElement("input");d.type="file";d.multiple=!0;d.style.display="hidden";document.body.appendChild(d);d.accept="."+b.join(",.");d.addEventListener("change",function(b){b=b.target.files;for(var c=b.length,d=0;d<c;++d)a.loadFile(b[d],void 0,void 0,void 0,{asTrajectory:f,firstModelOnly:h,cAlphaOnly:g})},!1);var e=(new NGL.ExportImageWidget(a)).setDisplay("none").attach(),
f=!1,h=!1,g=!1,b=UI.MenubarHelper.createOption,l=UI.MenubarHelper.createInput,k=UI.MenubarHelper.createCheckbox,n=UI.MenubarHelper.createDivider,b=[b("Open...",function(){d.click()}),b("Import...",function(){var b=new NGL.DirectoryListingWidget(a,"Import file",c,function(d){var e=d.path.split(".").pop().toLowerCase();-1!==c.indexOf(e)?a.loadFile(d.path,void 0,void 0,void 0,{asTrajectory:f,firstModelOnly:h,cAlphaOnly:g}):NGL.log("unknown filetype: "+e);b.dispose()});b.setOpacity("0.8").setLeft("50px").setTop("80px").attach()}),
...
NGL.MenubarHelpWidget=function(a){function b(){d.setOpacity("0.8").setLeft("50px").setTop("80px").setDisplay("block")}var c=(new NGL.PreferencesWidget(a)).setDisplay("none").attach(),d=(new NGL.OverviewWidget(a)).setDisplay("none").attach();a.preferences.getKey("overview")&&b();a=UI.MenubarHelper.createOption;var e=UI.MenubarHelper.createDivider;a=[a("Overview",b),a("Documentation",function(){window.open("../doc/index.html","_blank")}),e(),a("Unittests",function(){window.open("../test/unit/unittests.html",
"_blank")}),a("Benchmarks",function(){window.open("../test/bench/benchmarks.html","_blank")}),e(),a("Prefereces",function(){c.setOpacity("0.8").setLeft("50px").setTop("80px").setDisplay("block")},"sliders")];a=UI.MenubarHelper.createOptionsPanel(a);return UI.MenubarHelper.createMenuContainer("Help",a)};
...
l=(new UI.DisposeIcon).setMarginLeft("10px").setDisposeFunction(function(){a.removeAllComponents()}),k=(new UI.PopupMenu("cogs","Settings","window")).setIconTitle("settings").setMarginLeft("10px"),n=(new UI.Range(1,100,a.viewer.params.clipNear,1)).onInput(function(){a.viewer.setClip(n.getValue(),q.getValue())}),q=(new UI.Range(1,100,a.viewer.params.clipFar,1)).onInput(function(){a.viewer.setClip(n.getValue(),q.getValue())}),m=(new UI.Range(1,100,a.viewer.params.clipDist,1)).onInput(function(){a.viewer.params.clipDist=
m.getValue();a.viewer.requestRender()}),p=(new UI.Range(1,100,a.viewer.params.fogNear,1)).onInput(function(){a.viewer.setFog(null,null,p.getValue(),r.getValue())}),r=(new UI.Range(1,100,a.viewer.params.fogFar,1)).onInput(function(){a.viewer.setFog(null,null,p.getValue(),r.getValue())});k.addEntry("clip near",n).addEntry("clip far",q).addEntry("clip distance",m).addEntry("fog near",p).addEntry("fog far",r);b=(new UI.Panel).setClass("Panel Sticky").add(b,h,g,l,k);c.add(b,d);return c};
...
var c=["xtc","trr","dcd","netcdf","nc"],d=new NGL.DirectoryListingWidget(b,"Import trajectory",c,function(b){var e=b.path.split(".").pop().toLowerCase();-1!==c.indexOf(e)?(NGL.log(b),a.addTrajectory(b.path),d.dispose()):NGL.log("unknown trajectory type: "+e)});d.setOpacity("0.8").setLeft("50px").setTop("80px").attach()});b.signals.componentAdded.add(c);b.signals.componentRemoved.add(c);var k=(new UI.Select).setColor("#444").onChange(function(){a.superpose(b.compList[k.getValue()],!0);a.centerView();
k.setValue("");m.setMenuDisplay("none")});c();var n=(new UI.Button("calculate")).onClick(function(){a.structure.autoSS();a.rebuildRepresentations();m.setMenuDisplay("none")}),q=(new UI.Button("duplicate")).onClick(function(){b.addComponent(new NGL.StructureComponent(b,a.structure.clone(),{}));m.setMenuDisplay("none")}),m=(new UI.ComponentPanel(a)).setDisplay("inline-block").setMargin("0px").addMenuEntry("PDB file",d).addMenuEntry("Representation",g).addMenuEntry("Trajectory",l).addMenuEntry("Superpose",
k).addMenuEntry("SS",n).addMenuEntry("Structure",q).addMenuEntry("File",(new UI.Text(a.structure.path)).setMaxWidth("100px").setWordWrap("break-word"));e.addStatic(m).add(h).add(f);return e};
...
NGL.TrajectoryComponentWidget=function(a,b){function c(a){g.clear().add(k.setWidth("70px"));k.setRange(-1,a-1);q.setRange(-1,a-1);n.setValue(Math.ceil((a+1)/100));p.step=n.getValue();p.end=a}var d=a.signals,e=a.trajectory,f=(new UI.CollapsibleIconPanel("database")).setMarginLeft("20px"),h=new UI.Panel;d.representationAdded.add(function(a){h.add(new NGL.RepresentationComponentWidget(a,b))});d.disposed.add(function(){u.dispose();f.dispose()});var g=(new UI.Panel).setMarginLeft("10px").setDisplay("inline").add((new UI.Icon("spinner")).addClass("spin").setMarginRight("69px"));
d.gotNumframes.add(c);d.frameChanged.add(function(a){k.setValue(a);q.setValue(a);g.clear().add(k.setWidth("70px"))});var l=(new UI.EllipsisText(e.name)).setWidth("108px");f.addStatic(l);f.addStatic(g);var k=(new UI.Integer(-1)).setMarginLeft("5px").setWidth("70px").setRange(-1,-1).onChange(function(a){e.setFrame(k.getValue());u.setMenuDisplay("none")}),n=(new UI.Integer(1)).setWidth("30px").setRange(1,1E4).onChange(function(){p.step=n.getValue()}),l=new UI.Panel,q=(new UI.Range(-1,-1,-1,1)).setWidth("197px").setMargin("0px").setPadding("0px").setBorder("0px").onInput(function(a){a=
q.getValue();a!==e.currentFrame&&(e.player&&e.player._running?(e.setPlayer(),e.setFrame(a)):e.inProgress||e.setFrame(a))}),m=(new UI.Integer(50)).setWidth("30px").setRange(10,1E3).onChange(function(){p.timeout=m.getValue()}),p=new NGL.TrajectoryPlayer(e,n.getValue(),m.getValue(),0,e.numframes),r=(new UI.ToggleIcon(!0,"play","pause")).setMarginRight("10px").setMarginLeft("20px").setWidth("12px").setTitle("play").onClick(function(){p.toggle()});p.signals.startedRunning.add(function(){r.setTitle("pause").setValue(!1)});
p.signals.haltedRunning.add(function(){r.setTitle("play").setValue(!0)});l.add(r);l.add(q);f.add((new UI.SelectionPanel(e.selection)).setMarginLeft("20px").setInputWidth("194px"));var s=(new UI.Checkbox(e.params.centerPbc)).onChange(function(){a.setParameters({centerPbc:s.getValue()})}),t=(new UI.Checkbox(e.params.removePbc)).onChange(function(){a.setParameters({removePbc:t.getValue()})}),v=(new UI.Checkbox(e.params.superpose)).onChange(function(){a.setParameters({superpose:v.getValue()})});d.parametersChanged.add(function(a){s.setValue(a.centerPbc);
...
")");b=(new UI.Panel).setDisplay("block").add((new UI.Icon(b)).setWidth("20px")).add(new UI.Text(g)).onClick(function(){a.dir?f.getListing(a.path):d(a)});a.restricted&&b.add((new UI.Icon("lock")).setMarginLeft("5px"));l.add(b)}})});f.getListing(NGL.lastUsedDirectory);return h};
