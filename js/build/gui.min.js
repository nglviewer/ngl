...
NGL.ExportImageWidget=function(a){function b(a,b){f.add((new UI.Text(a)).setWidth("80px")).add(b||new UI.Panel).add(new UI.Break)}function c(b,c,d,e,f,k){var g=[];a.eachRepresentation(function(a){g.push(a.getParameters());var b=a.getParameters();void 0!==b.subdiv&&(b.subdiv=Math.max(25,b.subdiv));void 0!==b.radialSegments&&(b.radialSegments=Math.max(25,b.radialSegments));b.quality=null;a.rebuild(b)},NGL.StructureComponent);a.viewer.screenshot({factor:b,type:c,quality:d,antialias:e,transparent:f,trim:k,
onProgress:function(b,c,d){1===b&&m.setMax(c);b>=c?m.setIndeterminate():m.setValue(b);if(d){m.setDisplay("none");p.setDisplay("inline-block");var e=0;a.eachRepresentation(function(a){a.rebuild(g[e]);e+=1},NGL.StructureComponent)}}})}var d=new UI.OverlayPanel,e=(new UI.Panel).setBorderBottom("1px solid #555").setHeight("25px"),f=(new UI.Panel).setMarginTop("10px").setMinHeight("100px").setMaxHeight("500px").setOverflow("auto");e.add(new UI.Text("Image export"));e.add((new UI.Icon("times")).setMarginLeft("20px").setFloat("right").onClick(function(){d.setDisplay("none")}));
...
(new UI.Progress).setDisplay("none"),p=(new UI.Button("export")).onClick(function(){p.setDisplay("none");m.setDisplay("inline-block");setTimeout(function(){c(parseInt(g.getValue()),h.getValue(),parseFloat(k.getValue()),l.getValue(),n.getValue(),q.getValue())},50)});b("scale",g);b("type",h);b("quality",k);b("antialias",l);b("transparent",n);b("trim",q);f.add(new UI.Break,p,m);return d};
...
NGL.TrajectoryComponentWidget=function(a,b){function c(a){h.clear().add(l.setWidth("70px"));l.setRange(-1,a-1);q.setRange(-1,a-1);n.setValue(Math.ceil((a+1)/100));p.step=n.getValue();p.end=a}var d=a.signals,e=a.trajectory,f=(new UI.CollapsibleIconPanel("database")).setTitle("expand/collapse").setMarginLeft("20px"),g=new UI.Panel;d.representationAdded.add(function(a){g.add(new NGL.RepresentationComponentWidget(a,b))});var h=(new UI.Panel).setMarginLeft("10px").setDisplay("inline").add((new UI.Icon("spinner")).addClass("spin").setMarginRight("69px"));
d.gotNumframes.add(c);d.frameChanged.add(function(a){l.setValue(a);q.setValue(a);h.clear().add(l.setWidth("70px"))});var k=(new UI.Text(e.name)).setWidth("108px").setWordWrap("break-word");f.addStatic(k);f.addStatic(h);var l=(new UI.Integer(-1)).setMarginLeft("5px").setWidth("70px").setRange(-1,-1).onChange(function(a){e.setFrame(l.getValue());r.setMenuDisplay("none")}),n=(new UI.Integer(1)).setWidth("30px").setRange(1,1E4).onChange(function(){p.step=n.getValue()}),k=new UI.Panel,q=(new UI.Range(-1,
...
p.signals.startedRunning.add(function(){s.setTitle("pause").setValue(!1)});p.signals.haltedRunning.add(function(){s.setTitle("play").setValue(!0)});k.add(s);k.add(q);f.add((new UI.SelectionPanel(e.selection)).setMarginLeft("20px").setInputWidth("194px"));var t=(new UI.Checkbox(e.params.centerPbc)).onChange(function(){a.setParameters({centerPbc:t.getValue()});r.setMenuDisplay("none")}),u=(new UI.Checkbox(e.params.removePbc)).onChange(function(){a.setParameters({removePbc:u.getValue()});r.setMenuDisplay("none")}),
v=(new UI.Checkbox(e.params.superpose)).onChange(function(){a.setParameters({superpose:v.getValue()});r.setMenuDisplay("none")});d.parametersChanged.add(function(a){t.setValue(a.centerPbc);u.setValue(a.removePbc);v.setValue(a.superpose)});(new UI.Button("download")).onClick(function(){e.download(n.getValue())});d=(new UI.Button("add")).onClick(function(){a.addRepresentation();r.setMenuDisplay("none")});e.numframes&&c(e.numframes);var r=(new UI.PopupMenu("bars","Trajectory")).setMarginLeft("10px").setEntryLabelWidth("110px").addEntry("Path",
d).addEntry("Center",t).addEntry("Remove PBC",u).addEntry("Superpose",v).addEntry("Step",n).addEntry("Timeout",m).addEntry("File",(new UI.Text(e.trajPath)).setMaxWidth("100px").setWordWrap("break-word"));f.addStatic(r);f.add(k);f.add(g);return f};NGL.lastUsedDirectory="";NGL.DirectoryListing=function(){this.signals={listingLoaded:new signals.Signal}};
