...
NGL.createParameterInput=function(a){if(a){var b;"number"===a.type||"integer"===a.type?(b="number"===a.type?(new UI.Number(parseFloat(a.value)||NaN)).setPrecision(a.precision):new UI.Integer(parseInt(a.value)||NaN),b.setRange(a.min,a.max)):"range"===a.type?b=new UI.Range(a.min,a.max,a.value,a.step):"boolean"===a.type?b=new UI.Checkbox(a.value):"text"===a.type?b=new UI.Input(a.value):"select"===a.type?b=(new UI.Select).setWidth("").setOptions(a.options).setValue(a.value):"button"===a.type?b=(new UI.Button(a.label)).onClick(function(){a.value()}):
"color"===a.type?b=(new UI.ColorPopupMenu(a.label)).setValue(a.value):"hidden"!==a.type&&NGL.warn("NGL.createParameterInput: unknown parameter type '"+a.type+"'");return b}};
...
NGL.MenubarViewWidget=function(a){var b=UI.MenubarHelper.createOption,c=UI.MenubarHelper.createDivider,b=[b("Light theme",function(){a.setParameters({theme:"light"})}),b("Dark theme",function(){a.setParameters({theme:"dark"})}),c(),b("Full screen",function(){a.toggleFullscreen(document.body)},"expand"),b("Center",function(){a.centerView()},"bullseye"),c(),b("Orientation",function(){window.prompt("Orientation",JSON.stringify(a.viewer.getOrientation()))})],b=UI.MenubarHelper.createOptionsPanel(b);return UI.MenubarHelper.createMenuContainer("View",
b)};NGL.MenubarExamplesWidget=function(a){var b=UI.MenubarHelper.createOption,c=UI.MenubarHelper.createDivider,d=[];NGL.ExampleRegistry.names.sort().forEach(function(e){if("__divider__"===e)d.push(c());else if("_"!==e.charAt(0)){var g=b(e,function(){NGL.ExampleRegistry.load(e,a)});d.push(g)}});var e=UI.MenubarHelper.createOptionsPanel(d);return UI.MenubarHelper.createMenuContainer("Examples",e)};
...
d[b])}var g=a.parameters[b];void 0===g.label&&(g.label=b);var h=NGL.createParameterInput(g);if(h){a.preferences.signals.keyChanged.add(function(a,c){a===b&&h.setValue(c)});if("range"===g.type)h.onInput(c);else h.onChange(c);d.add((new UI.Text(b)).setWidth("90px")).add(h).add(new UI.Break)}});return b};
...
k=(new UI.DisposeIcon).setMarginLeft("10px").setDisposeFunction(function(){a.removeAllComponents()}),l=(new UI.PopupMenu("cogs","Settings","window")).setIconTitle("settings").setMarginLeft("10px"),n=(new UI.Panel).setDisplay("inline").add((new UI.Icon("spinner")).addClass("spin").setMarginLeft("45px"));a.tasks.signals.countChanged.add(function(a,b){if(0<b)r.add(n);else try{r.remove(n)}catch(c){}});["clipNear","clipFar","clipDist","fogNear","fogFar"].forEach(function(b){function c(){var d={};d[b]=
e.getValue();a.setParameters(d)}var d=a.parameters[b];void 0===d.label&&(d.label=b);var e=NGL.createParameterInput(d);if(e){a.signals.parametersChanged.add(function(a){e.setValue(a[b])});if("range"===d.type)e.onInput(c);else e.onChange(c);l.addEntry(b,e)}});var r=(new UI.Panel).setClass("Panel Sticky").add(b,g,h,k,l);c.add(r,d);return c};
...
Object.keys(a.repr.parameters).forEach(function(b){var d=a.repr,e=d.parameters[b];if(e){void 0===e.label&&(e.label=b);var f=NGL.createParameterInput(e);f&&(c.parametersChanged.add(function(a){f.setValue(a[b])}),f.onChange(function(){var c={};c[b]=f.getValue();a.setParameters(c);d.viewer.requestRender()}),h.addEntry(b,f))}});d.addStatic(h);return d};
