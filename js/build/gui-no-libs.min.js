...
UI.Integer=function(a){UI.Element.call(this);var b=this,c=document.createElement("input");c.className="Number";c.value="0.00";c.addEventListener("keydown",function(a){a.stopPropagation()},!1);this.min=-Infinity;this.max=Infinity;this.step=1;this.dom=c;this.setValue(a);var d=document.createEvent("HTMLEvents");d.initEvent("change",!0,!0);var e=0,f=0,g=[0,0],h=[0,0],l=function(a){var k=c.value;g=[a.clientX,a.clientY];e+=g[0]-h[0]-(g[1]-h[1]);var q=50;a.shiftKey&&(q=5);a.ctrlKey&&(q=500);c.value=Math.min(b.max,
Math.max(b.min,f+e/q*b.step))|0;k!==c.value&&c.dispatchEvent(d);h=[a.clientX,a.clientY]},k=function(a){document.removeEventListener("mousemove",l,!1);document.removeEventListener("mouseup",k,!1);2>Math.abs(e)&&(c.focus(),c.select())};c.addEventListener("mousedown",function(a){a.preventDefault();e=0;f=parseFloat(c.value);h=[a.clientX,a.clientY];document.addEventListener("mousemove",l,!1);document.addEventListener("mouseup",k,!1)},!1);c.addEventListener("change",function(a){a=parseInt(c.value);!1===
...
this.add(this.menu);this.setClass("").setDisplay("inline");return this};UI.ColorPopupMenu.prototype=Object.create(UI.Panel.prototype);UI.ColorPopupMenu.prototype.setScheme=function(a){a=a||"";this.iconText.setValue(a.charAt(0).toUpperCase());this.schemeSelector.setValue(a);"color"!==a&&this.setColor("#888888");return this};UI.ColorPopupMenu.prototype.getScheme=function(){return this.schemeSelector.getValue()};
...
f=!1,g=!1,h=!1,b=UI.MenubarHelper.createOption,l=UI.MenubarHelper.createInput,k=UI.MenubarHelper.createCheckbox,m=UI.MenubarHelper.createDivider,b=[b("Open...",function(){d.click()}),b("Import...",function(){var b=new NGL.DirectoryListingWidget(a,"Import file",c,function(d){var e=d.path.split(".").pop().toLowerCase();-1!==c.indexOf(e)?a.loadFile(d.path,void 0,void 0,void 0,{asTrajectory:f,firstModelOnly:g,cAlphaOnly:h}):NGL.log("unknown filetype: "+e);b.dispose()});b.setOpacity("0.8").setLeft("50px").setTop("80px").attach()}),
...
l=(new UI.DisposeIcon).setMarginLeft("10px").setDisposeFunction(function(){a.removeAllComponents()}),k=(new UI.PopupMenu("cogs","Settings","window")).setIconTitle("settings").setMarginLeft("10px"),m=(new UI.Range(1,100,a.viewer.params.clipNear,1)).onInput(function(){a.viewer.setClip(m.getValue(),p.getValue())}),p=(new UI.Range(1,100,a.viewer.params.clipFar,1)).onInput(function(){a.viewer.setClip(m.getValue(),p.getValue())}),q=(new UI.Range(1,100,a.viewer.params.clipDist,1)).onInput(function(){a.viewer.params.clipDist=
q.getValue();a.viewer.requestRender()}),n=(new UI.Range(1,100,a.viewer.params.fogNear,1)).onInput(function(){a.viewer.setFog(null,null,n.getValue(),s.getValue())}),s=(new UI.Range(1,100,a.viewer.params.fogFar,1)).onInput(function(){a.viewer.setFog(null,null,n.getValue(),s.getValue())});k.addEntry("clip near",m).addEntry("clip far",p).addEntry("clip distance",q).addEntry("fog near",n).addEntry("fog far",s);b=(new UI.Panel).setClass("Panel Sticky").add(b,g,h,l,k);c.add(b,d);return c};
...
b))});e.add((new UI.SelectionPanel(a.selection)).setMarginLeft("20px").setInputWidth("214px"));var d=(new UI.Button("export")).onClick(function(){var b=new Blob([a.structure.toPdb()],{type:"text/plain"});NGL.download(b,"structure.pdb");n.setMenuDisplay("none")}),h=(new UI.Select).setColor("#444").setOptions(function(){var a={"":"[ add ]"},b;for(b in NGL.representationTypes)a[b]=b;return a}()).onChange(function(){a.addRepresentation(h.getValue());h.setValue("");n.setMenuDisplay("none")}),l=(new UI.Select).setColor("#444").setOptions(function(){var b=
{__AU:"AU"};Object.keys(a.structure.biomolDict).forEach(function(a){b[a]=a});return b}()).setValue(a.structure.defaultAssembly).onChange(function(){a.structure.setDefaultAssembly(l.getValue());a.rebuildRepresentations();n.setMenuDisplay("none")}),k=(new UI.Button("import")).onClick(function(){n.setMenuDisplay("none");var c=["xtc","trr","dcd","netcdf","nc"],d=new NGL.DirectoryListingWidget(b,"Import trajectory",c,function(b){var e=b.path.split(".").pop().toLowerCase();-1!==c.indexOf(e)?(NGL.log(b),
a.addTrajectory(b.path),d.dispose()):NGL.log("unknown trajectory type: "+e)});d.setOpacity("0.8").setLeft("50px").setTop("80px").attach()});b.signals.componentAdded.add(c);b.signals.componentRemoved.add(c);var m=(new UI.Select).setColor("#444").onChange(function(){a.superpose(b.compList[m.getValue()],!0);a.centerView();m.setValue("");n.setMenuDisplay("none")});c();var p=(new UI.Button("calculate")).onClick(function(){a.structure.autoSS();a.rebuildRepresentations();n.setMenuDisplay("none")}),q=(new UI.Button("duplicate")).onClick(function(){b.addComponent(new NGL.StructureComponent(b,
a.structure.clone(),{}));n.setMenuDisplay("none")}),n=(new UI.ComponentPanel(a)).setDisplay("inline-block").setMargin("0px").addMenuEntry("PDB file",d).addMenuEntry("Representation",h).addMenuEntry("Assembly",l).addMenuEntry("Trajectory",k).addMenuEntry("Superpose",m).addMenuEntry("SS",p).addMenuEntry("Structure",q).addMenuEntry("File",(new UI.Text(a.structure.path)).setMaxWidth("100px").setWordWrap("break-word"));e.addStatic(n).add(g).add(f);return e};
...
(a.parent instanceof NGL.StructureComponent||a.parent instanceof NGL.TrajectoryComponent)&&a.repr.selection instanceof NGL.Selection&&d.add((new UI.SelectionPanel(a.repr.selection)).setMarginLeft("20px").setInputWidth("194px"));var l=(new UI.PopupMenu("bars","Representation")).setMarginLeft("45px").setEntryLabelWidth("110px");Object.keys(a.repr.parameters).forEach(function(b){var d=a.repr,e,f=d.parameters[b];f&&("number"===f.type||"integer"===f.type?(e="number"===f.type?(new UI.Number(parseFloat(d[b])||
NaN)).setPrecision(f.precision):new UI.Integer(parseInt(d[b])||NaN),e.setRange(f.min,f.max)):"boolean"===f.type?e=new UI.Checkbox(d[b]):"text"===f.type?e=new UI.Input(d[b]):"select"===f.type?e=(new UI.Select).setWidth("").setOptions(f.options).setValue(d[b]):"button"===f.type&&(e=(new UI.Button(b)).onClick(function(){d[b]()})),e&&(c.parametersChanged.add(function(a){e.setValue(a[b])}),e.onChange(function(){var a={};a[b]=e.getValue();d.setParameters(a);d.viewer.render()}),l.addEntry(b,e)))});d.addStatic(l);
return d};
NGL.TrajectoryComponentWidget=function(a,b){function c(a){h.clear().add(k.setWidth("70px"));k.setRange(-1,a-1);p.setRange(-1,a-1);m.setValue(Math.ceil((a+1)/100));r.step=m.getValue();r.end=a}var d=a.signals,e=a.trajectory,f=(new UI.CollapsibleIconPanel("database")).setMarginLeft("20px"),g=new UI.Panel;d.representationAdded.add(function(a){g.add(new NGL.RepresentationComponentWidget(a,b))});d.disposed.add(function(){u.dispose();f.dispose()});var h=(new UI.Panel).setMarginLeft("10px").setDisplay("inline").add((new UI.Icon("spinner")).addClass("spin").setMarginRight("69px"));d.gotNumframes.add(c);
d.frameChanged.add(function(a){k.setValue(a);p.setValue(a);h.clear().add(k.setWidth("70px"))});var l=(new UI.EllipsisText(e.name)).setWidth("108px");f.addStatic(l);f.addStatic(h);var k=(new UI.Integer(-1)).setMarginLeft("5px").setWidth("70px").setRange(-1,-1).onChange(function(a){e.setFrame(k.getValue());u.setMenuDisplay("none")}),m=(new UI.Integer(1)).setWidth("30px").setRange(1,1E4).onChange(function(){r.step=m.getValue()}),l=new UI.Panel,p=(new UI.Range(-1,-1,-1,1)).setWidth("197px").setMargin("0px").setPadding("0px").setBorder("0px").onInput(function(a){a=
p.getValue();a!==e.currentFrame&&(e.player&&e.player._running?(e.setPlayer(),e.setFrame(a)):e.inProgress||e.setFrame(a))}),q=(new UI.Select).setColor("#444").setOptions({"":"none",linear:"linear",spline:"spline"}).onChange(function(){r.interpolateType=q.getValue()}),n=(new UI.Integer(5)).setWidth("30px").setRange(1,50).onChange(function(){r.interpolateStep=n.getValue()}),s=(new UI.Integer(50)).setWidth("30px").setRange(10,1E3).onChange(function(){r.timeout=s.getValue()}),r=new NGL.TrajectoryPlayer(e,
m.getValue(),s.getValue(),0,e.numframes),t=(new UI.ToggleIcon(!0,"play","pause")).setMarginRight("10px").setMarginLeft("20px").setCursor("pointer").setWidth("12px").setTitle("play").onClick(function(){r.toggle()});r.signals.startedRunning.add(function(){t.setTitle("pause").setValue(!1)});r.signals.haltedRunning.add(function(){t.setTitle("play").setValue(!0)});l.add(t);l.add(p);var y=(new UI.Select).setColor("#444").setOptions({forward:"forward",backward:"backward"}).onChange(function(){r.direction=
y.getValue()}),z=(new UI.Select).setColor("#444").setOptions({loop:"loop",once:"once"}).onChange(function(){r.mode=z.getValue()});f.add((new UI.SelectionPanel(e.selection)).setMarginLeft("20px").setInputWidth("194px"));var v=(new UI.Checkbox(e.params.centerPbc)).onChange(function(){a.setParameters({centerPbc:v.getValue()})}),w=(new UI.Checkbox(e.params.removePbc)).onChange(function(){a.setParameters({removePbc:w.getValue()})}),x=(new UI.Checkbox(e.params.superpose)).onChange(function(){a.setParameters({superpose:x.getValue()})});
d.parametersChanged.add(function(a){v.setValue(a.centerPbc);w.setValue(a.removePbc);x.setValue(a.superpose)});(new UI.Button("download")).onClick(function(){e.download(m.getValue())});var d=(new UI.Button("add")).onClick(function(){a.addRepresentation()}),A=(new UI.DisposeIcon).setDisposeFunction(function(){a.parent.removeTrajectory(a)});e.numframes&&c(e.numframes);var u=(new UI.PopupMenu("bars","Trajectory")).setMarginLeft("10px").setEntryLabelWidth("130px").addEntry("Path",d).addEntry("Center",
v).addEntry("Remove PBC",w).addEntry("Superpose",x).addEntry("Step size",m).addEntry("Interpolation type",q).addEntry("Interpolation steps",n).addEntry("Play timeout",s).addEntry("Play direction",y).addEntry("Play mode",z).addEntry("File",(new UI.Text(e.trajPath)).setMaxWidth("100px").setWordWrap("break-word")).addEntry("Dispose",A);f.addStatic(u);f.add(l);f.add(g);return f};NGL.lastUsedDirectory="";NGL.DirectoryListing=function(){this.signals={listingLoaded:new signals.Signal}};
