...
UI.Integer=function(a){UI.Element.call(this);var b=this,c=document.createElement("input");c.className="Number";c.value="0.00";c.addEventListener("keydown",function(a){a.stopPropagation()},!1);this.min=-Infinity;this.max=Infinity;this.step=1;this.dom=c;this.setValue(a);var d=document.createEvent("HTMLEvents");d.initEvent("change",!0,!0);var e=0,f=0,g=[0,0],h=[0,0],l=function(a){var k=c.value;g=[a.clientX,a.clientY];e+=g[0]-h[0]-(g[1]-h[1]);var m=50;a.shiftKey&&(m=5);a.ctrlKey&&(m=500);c.value=Math.min(b.max,
Math.max(b.min,f+e/m*b.step))|0;k!==c.value&&c.dispatchEvent(d);h=[a.clientX,a.clientY]},k=function(a){document.removeEventListener("mousemove",l,!1);document.removeEventListener("mouseup",k,!1);2>Math.abs(e)&&(c.focus(),c.select())};c.addEventListener("mousedown",function(a){a.preventDefault();e=0;f=parseFloat(c.value);h=[a.clientX,a.clientY];document.addEventListener("mousemove",l,!1);document.addEventListener("mouseup",k,!1)},!1);c.addEventListener("change",function(a){a=parseInt(c.value);!1===
...
this.pickerIndicator.dom);return this};UI.ColorPicker.prototype=Object.create(UI.Panel.prototype);UI.ColorPicker.prototype.setValue=function(a){this._settingValue=!0;this.colorPicker.setHex(a);this._settingValue=!1;return this};UI.ColorPicker.prototype.getValue=function(){return this.hex};
...
"color"!==a.schemeSelector.getValue()&&a.menu.setMenuDisplay("none");a.dom.dispatchEvent(b)});this.colorInput=(new UI.Input).onChange(function(){a.setScheme("color");a.setColor(a.colorInput.getValue());a.dom.dispatchEvent(b)});this.colorPicker=(new UI.ColorPicker).setDisplay("inline-block").onChange(function(c){a.setScheme("color");a.setColor(a.colorPicker.getValue());a.dom.dispatchEvent(b)});this.menu.addEntry("Scheme",this.schemeSelector).addEntry("Input",this.colorInput).addEntry("Picker",this.colorPicker);
this.add(this.menu);this.setClass("").setDisplay("inline");return this};UI.ColorPopupMenu.prototype=Object.create(UI.Panel.prototype);UI.ColorPopupMenu.prototype.setScheme=function(a){this.iconText.setValue(a.charAt(0).toUpperCase());this.schemeSelector.setValue(a);"color"!==a&&this.setColor("#888888");return this};UI.ColorPopupMenu.prototype.getScheme=function(){return this.schemeSelector.getValue()};
...
NGL.MenubarFileWidget=function(a){var b="pdb ent gro cif mmcif mrc ccp4 map cube obj ply ngl ngz gz lzma bz2 zip".split(" "),c=b+["ngl"],d=document.createElement("input");d.type="file";d.multiple=!0;d.style.display="hidden";document.body.appendChild(d);d.accept="."+b.join(",.");d.addEventListener("change",function(b){b=b.target.files;for(var c=b.length,d=0;d<c;++d)a.loadFile(b[d],void 0,void 0,void 0,{asTrajectory:f,firstModelOnly:g,cAlphaOnly:h})},!1);var e=(new NGL.ExportImageWidget(a)).setDisplay("none").attach(),
f=!1,g=!1,h=!1,b=UI.MenubarHelper.createOption,l=UI.MenubarHelper.createInput,k=UI.MenubarHelper.createCheckbox,n=UI.MenubarHelper.createDivider,b=[b("Open...",function(){d.click()}),b("Import...",function(){var b=new NGL.DirectoryListingWidget(a,"Import file",c,function(d){var e=d.path.split(".").pop().toLowerCase();-1!==c.indexOf(e)?a.loadFile(d.path,void 0,void 0,void 0,{asTrajectory:f,firstModelOnly:g,cAlphaOnly:h}):NGL.log("unknown filetype: "+e);b.dispose()});b.setOpacity("0.8").setLeft("50px").setTop("80px").attach()}),
l("PDB",function(b){13===b.keyCode&&(a.loadFile("rcsb://"+b.target.value,void 0,void 0,void 0,{asTrajectory:f,firstModelOnly:g,cAlphaOnly:h}),b.target.value="")}),k("asTrajectory",!1,function(a){f=a.target.checked}),k("firstModelOnly",!1,function(a){g=a.target.checked}),k("cAlphaOnly",!1,function(a){h=a.target.checked}),n(),b("Screenshot",function(){a.viewer.screenshot({factor:1,type:"image/png",quality:1,antialias:!0,transparent:!1,trim:!1})},"camera"),b("Export image...",function(){e.setOpacity("0.8").setLeft("50px").setTop("80px").setDisplay("block")})],
...
m.getValue();a.viewer.requestRender()}),s=(new UI.Range(1,100,a.viewer.params.fogNear,1)).onInput(function(){a.viewer.setFog(null,null,s.getValue(),r.getValue())}),r=(new UI.Range(1,100,a.viewer.params.fogFar,1)).onInput(function(){a.viewer.setFog(null,null,s.getValue(),r.getValue())});k.addEntry("clip near",n).addEntry("clip far",q).addEntry("clip distance",m).addEntry("fog near",s).addEntry("fog far",r);b=(new UI.Panel).setClass("Panel Sticky").add(b,g,h,l,k);c.add(b,d);return c};
...
NGL.RepresentationComponentWidget=function(a,b){var c=a.signals,d=(new UI.CollapsibleIconPanel("bookmark")).setMarginLeft("20px");c.visibilityChanged.add(function(a){f.setValue(a)});c.colorChanged.add(function(a){h.setValue(a)});c.disposed.add(function(){l.dispose();h.dispose();d.dispose()});var e=(new UI.EllipsisText(a.repr.type)).setWidth("80px"),f=(new UI.ToggleIcon(a.visible,"eye","eye-slash")).setTitle("hide/show").setCursor("pointer").setMarginLeft("25px").onClick(function(){a.setVisibility(!a.visible)}),
g=(new UI.DisposeIcon).setMarginLeft("10px").setDisposeFunction(function(){a.dispose()}),h=(new UI.ColorPopupMenu).setMarginLeft("10px").setValue(a.repr.color).onChange(function(){var b=new THREE.Color;return function(c){c=h.getScheme();"color"===c?(b.setStyle(h.getColor()),a.setColor(b.getHex())):a.setColor(c);a.viewer.requestRender()}}());a.parent instanceof NGL.SurfaceComponent&&h.schemeSelector.setOptions({"":"",value:"value",color:"color"});d.addStatic(e).addStatic(f).addStatic(g).addStatic(h);
(a.parent instanceof NGL.StructureComponent||a.parent instanceof NGL.TrajectoryComponent)&&d.add((new UI.SelectionPanel(a.repr.selection)).setMarginLeft("20px").setInputWidth("194px"));var l=(new UI.PopupMenu("bars","Representation")).setMarginLeft("45px").setEntryLabelWidth("110px");Object.keys(a.repr.parameters).forEach(function(b){var d=a.repr,e,f=d.parameters[b];f&&("number"===f.type||"integer"===f.type?(e="number"===f.type?(new UI.Number(parseFloat(d[b])||NaN)).setPrecision(f.precision):new UI.Integer(parseInt(d[b])||
NaN),e.setRange(f.min,f.max)):"boolean"===f.type?e=new UI.Checkbox(d[b]):"text"===f.type?e=new UI.Input(d[b]):"select"===f.type?e=(new UI.Select).setWidth("").setOptions(f.options).setValue(d[b]):"button"===f.type&&(e=(new UI.Button(b)).onClick(function(){d[b]()})),e&&(c.parametersChanged.add(function(a){e.setValue(a[b])}),e.onChange(function(){var a={};a[b]=e.getValue();d.setParameters(a);d.viewer.render()}),l.addEntry(b,e)))});d.addStatic(l);return d};
NGL.TrajectoryComponentWidget=function(a,b){function c(a){h.clear().add(k.setWidth("70px"));k.setRange(-1,a-1);q.setRange(-1,a-1);n.setValue(Math.ceil((a+1)/100));p.step=n.getValue();p.end=a}var d=a.signals,e=a.trajectory,f=(new UI.CollapsibleIconPanel("database")).setMarginLeft("20px"),g=new UI.Panel;d.representationAdded.add(function(a){g.add(new NGL.RepresentationComponentWidget(a,b))});d.disposed.add(function(){u.dispose();f.dispose()});var h=(new UI.Panel).setMarginLeft("10px").setDisplay("inline").add((new UI.Icon("spinner")).addClass("spin").setMarginRight("69px"));
...
q.getValue();a!==e.currentFrame&&(e.player&&e.player._running?(e.setPlayer(),e.setFrame(a)):e.inProgress||e.setFrame(a))}),m=(new UI.Select).setColor("#444").setOptions({"":"none",linear:"linear",spline:"spline"}).onChange(function(){p.interpolateType=m.getValue()}),s=(new UI.Integer(5)).setWidth("30px").setRange(1,50).onChange(function(){p.interpolateStep=s.getValue()}),r=(new UI.Integer(50)).setWidth("30px").setRange(10,1E3).onChange(function(){p.timeout=r.getValue()}),p=new NGL.TrajectoryPlayer(e,
n.getValue(),r.getValue(),0,e.numframes),t=(new UI.ToggleIcon(!0,"play","pause")).setMarginRight("10px").setMarginLeft("20px").setCursor("pointer").setWidth("12px").setTitle("play").onClick(function(){p.toggle()});p.signals.startedRunning.add(function(){t.setTitle("pause").setValue(!1)});p.signals.haltedRunning.add(function(){t.setTitle("play").setValue(!0)});l.add(t);l.add(q);var y=(new UI.Select).setColor("#444").setOptions({forward:"forward",backward:"backward"}).onChange(function(){p.direction=
y.getValue()}),z=(new UI.Select).setColor("#444").setOptions({loop:"loop",once:"once"}).onChange(function(){p.mode=z.getValue()});f.add((new UI.SelectionPanel(e.selection)).setMarginLeft("20px").setInputWidth("194px"));var v=(new UI.Checkbox(e.params.centerPbc)).onChange(function(){a.setParameters({centerPbc:v.getValue()})}),w=(new UI.Checkbox(e.params.removePbc)).onChange(function(){a.setParameters({removePbc:w.getValue()})}),x=(new UI.Checkbox(e.params.superpose)).onChange(function(){a.setParameters({superpose:x.getValue()})});
d.parametersChanged.add(function(a){v.setValue(a.centerPbc);w.setValue(a.removePbc);x.setValue(a.superpose)});(new UI.Button("download")).onClick(function(){e.download(n.getValue())});var d=(new UI.Button("add")).onClick(function(){a.addRepresentation()}),A=(new UI.DisposeIcon).setDisposeFunction(function(){a.parent.removeTrajectory(a)});e.numframes&&c(e.numframes);var u=(new UI.PopupMenu("bars","Trajectory")).setMarginLeft("10px").setEntryLabelWidth("130px").addEntry("Path",d).addEntry("Center",
v).addEntry("Remove PBC",w).addEntry("Superpose",x).addEntry("Step size",n).addEntry("Interpolation type",m).addEntry("Interpolation steps",s).addEntry("Play timeout",r).addEntry("Play direction",y).addEntry("Play mode",z).addEntry("File",(new UI.Text(e.trajPath)).setMaxWidth("100px").setWordWrap("break-word")).addEntry("Dispose",A);f.addStatic(u);f.add(l);f.add(g);return f};NGL.lastUsedDirectory="";NGL.DirectoryListing=function(){this.signals={listingLoaded:new signals.Signal}};
