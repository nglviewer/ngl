...
describe('utils/bitarray', function () {
  describe('BitArray', function () {
    ...
    it('isRangeSet', function () {
      ...
      // test away from first word
      ba = new BitArray(2000)
      ba.setRange(1000, 1500)
      expect(ba.isRangeSet(1000, 1500)).toBeTruthy()
    })
    ...
    it('setRange', function () {
      ...
      expect(ba.get(40)).toBeTruthy()
      expect(ba.get(70)).toBeTruthy()
      expect(ba.get(80)).toBeFalsy()
      expect(ba.isRangeSet(10, 70)).toBeTruthy()
      expect(ba.isRangeSet(0, 8)).toBeFalsy()
      expect(ba.isRangeSet(10, 90)).toBeFalsy()
      expect(ba.isRangeSet(0, 11)).toBeFalsy()
      expect(ba.getSize()).toBe(61)

      ba.clearAll()
      ba.setRange(10, 30) // within single word
      expect(ba.get(0)).toBeFalsy()
      expect(ba.get(10)).toBeTruthy()
      expect(ba.get(20)).toBeTruthy()
      expect(ba.get(30)).toBeTruthy()
      expect(ba.get(31)).toBeFalsy()
      expect(ba.isRangeSet(10, 30)).toBeTruthy()
      expect(ba.isRangeSet(0, 8)).toBeFalsy()
      expect(ba.isRangeSet(20, 70)).toBeFalsy()
      expect(ba.isRangeSet(0, 20)).toBeFalsy()
      expect(ba.getSize()).toBe(21)

      ba.clearAll()
      ba.setRange(20, 40) // length less than one word but span two words
      expect(ba.get(0)).toBeFalsy()
      expect(ba.get(20)).toBeTruthy()
      expect(ba.get(33)).toBeTruthy()
      expect(ba.get(40)).toBeTruthy()
      expect(ba.get(45)).toBeFalsy()
      expect(ba.isRangeSet(0, 18)).toBeFalsy()
      expect(ba.isRangeSet(0, 18)).toBeFalsy()
      expect(ba.isRangeSet(30, 90)).toBeFalsy()
      expect(ba.isRangeSet(0, 30)).toBeFalsy()
      expect(ba.getSize()).toBe(21)
    })
  })
})
